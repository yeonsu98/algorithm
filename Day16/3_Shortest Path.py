...
# 알고리즘 순서)
# 1. 출발 노드를 정한다
# 2. 최단 거리 테이블(= DP-Table)을 초기화
# 3. 방문하지 않는 노드 중 비용이 가장 짧은 노드를 선택 후 이동
# 4. 이동 후 해당 노드를 거쳐 다른 노드로 가는 비용을 계산 후 DP를 갱신
# 5. 3 ~ 4를 반복
...
8. 방문하지 않은 마지막 6노드를 방문
하지만, 6노드는 더 이상 이동할 노드가 없다 -> 끝

최종 DP 테이블을 살펴보면 아래와 같다
[1, 2, 3, 4, 5, 6]
[0, 2, 3, 1, 2, 4]
